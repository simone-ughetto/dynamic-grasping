...
graspable cylinder placed on top of the cart in an empty world.

This file has been reorganized to follow a similar structure to 
xsarm_moveit.launch.py. The spawn entities (cart and cylinder) are launched 
sequentially using event handlers, ensuring that the next spawn action occurs 
only when the previous one has completed. MoveIt is also launched with this file.
"""
...
def launch_setup(context, *args, **kwargs):
    ...
    cart_description = {'robot_description': Command(['xacro ', cart_xacro])}
    ...
    # -------------------------------------------------------------------------
    # 5. Cylinder Description and (Ephemeral) Spawn Node
    # -------------------------------------------------------------------------
    ...
    cylinder_description = {'robot_description': Command(['xacro ', cylinder_xacro])}
    ...
    # The spawn node is ephemeral; it exits once the object is spawned.
    spawn_cylinder = Node(
        package='gazebo_ros',
        executable='spawn_entity.py',
        name='spawn_cylinder',
    ...
    # -------------------------------------------------------------------------
    # 6. Additional Nodes (Teleop, TF Transforms, Controllers, etc.)
    # -------------------------------------------------------------------------
    teleop_node = Node(
    ...
    # -------------------------------------------------------------------------
    # 7. Define Event Handlers for Sequential Execution
    # -------------------------------------------------------------------------
    # (a) When spawn_cart exits, trigger spawn_cylinder.
    ...
    # (b) When spawn_cylinder exits, trigger the remaining nodes:
    ...
    # -------------------------------------------------------------------------
    # 8. Assemble the Launch Description
    # -------------------------------------------------------------------------
