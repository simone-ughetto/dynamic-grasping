...
OptitrackDriverNode::process_frame(sFrameOfMocapData * data)
{
  ...
  if (mocap4r2_rigid_body_pub_->get_subscription_count() > 0) {
    ...
    for (int i = 0; i < data->nRigidBodies; i++) {
      ...
      // Find the rigid body name from the data descriptions
      rb.rigid_body_name = "UnknownRigidBody_" + std::to_string(data->RigidBodies[i].ID); // Default name
      if (data_descriptions) {
        for (int j = 0; j < data_descriptions->nDataDescriptions; j++) {
          if (data_descriptions->arrDataDescriptions[j].type == Descriptor_RigidBody) {
            sRigidBodyDescription* pRB = data_descriptions->arrDataDescriptions[j].Data.RigidBodyDescription;
            if (pRB && pRB->ID == data->RigidBodies[i].ID) {
              rb.rigid_body_name = pRB->szName;
      ...
      // rb.rigid_body_name = std::to_string(data->RigidBodies[i].ID); // Original line
